// -----------------------------------------------------------------------------
// The demo implementation of OrdersRepository.
//
// This is the original in-memory mock, kept alive behind the VITE_DEMO_MODE
// flag so the board can be shown without a database — on a laptop with no
// network, in a classroom, or when the Supabase project is asleep.
//
// It is a peer of the Supabase implementation, not a fallback: nothing selects
// it automatically. ordersRepository.ts reads the flag and picks one, and the
// components cannot tell which one they got.
//
// Seed data comes from mockSeed.ts, which mirrors migration 001 row for row.
// -----------------------------------------------------------------------------

import type { MenuItem, Order, OrderItem } from '../types/models'
